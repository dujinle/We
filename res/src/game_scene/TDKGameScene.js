var tdkGameLayer = cc.Layer.extend({
	...
	//发牌的变量标志
	startFaPaiPosition:null,
	startFaPaiPosition_t:null,
	//发牌的轮圈数
	round_num:null,
	shaiZiRound:null,
	shaiZiNum:null,
	...
	ctor:function(){
		...
		this.shaiZiRound = 1;
		this.shaiZiNum = 1;
		this.firstFapaiNum = 3;
		this.startFaPaiPosition = 1;
		this.jiaZhuMenuFlag = true;
		this.biPaiMenuItemFlag = false;
		this.startFaPaiPosition_t = this.startFaPaiPosition;
		...
		console.log("start go into tdkGameLayer......");
	...
	initGameBackground:function(){
		...
		//logo
		var logoSprite = new cc.Sprite(res.TDKLogo_png);
	...
	initGameMenu:function(){
		...
		this.genZhuMenuItem=new cc.MenuItemSprite(genZhuSprite,genZhuSprite1,genZhuSprite2,this.game_follow,this);
	...
	initMenuItemVisibleBiPai:function(){
		//正在游戏中则新加入的玩家不可以准备
		this.zhunBeiMenuItem.setEnabled(false);
		this.biPaiMenuItem.setEnabled(false);
		this.qiPaiMenuItem.setEnabled(false);
	...
	initMenuItemVisibleCallbackAfterDealCard:function(){
		...
		for(var i = 0;i < g_players.length;i++){
			...
			if(player.positionServer == g_myselfPlayerPos){
				if(player.checkCard == true){
					this.kaiPaiMenuItem.setEnabled(false);
				}else{
					this.kaiPaiMenuItem.setEnabled(true);
				}

				if(this.currentGetPowerPlayerPosition == player.positionServer){
					if(this.biPaiMenuItemFlag == true){
	...
	initPlayersAndPlayer_noPower:function(){
		...
		for(var i=0;i<g_playerData.length;i++){
			var	player = new TDKPlayers(g_playerData[i]);
			if(this.roomState == 1){
	...
	getPlayerCardPosition:function(player,m){
		console.log("getPlayerCardPosition : playerID" + player.id + " card id:" + m);
		switch (player.playerPosition){
			case 1:
				var x = player.spritePhotoMobile.getPositionX() +
						player.spritePhotoMobile.getContentSize().width +
						g_dealCardBack.getContentSize().width/2*(m);
				var y = player.spritePhotoMobile.getPositionY();
				return cc.p(x,y);
			case 2:
				var x = player.spritePhotoMobile.getPositionX() - (5-m)*30;
				var y = player.spritePhotoMobile.getPositionY() + player.spritePhotoMobile.getContentSize().height + 30;
				return cc.p(x,y);
			case 3:
				var x = player.spritePhotoMobile.getPositionX() - (5-m)*30;
				var y =	player.spritePhotoMobile.getPositionY() - player.spritePhotoMobile.getContentSize().height - 30;
				return cc.p(x,y);
			case 4:
				var x = player.spritePhotoMobile.getPositionX() + m*30;
				var y = player.spritePhotoMobile.getPositionY() - player.spritePhotoMobile.getContentSize().height - 30;
				return cc.p(x,y);
			case 5:
				var x = player.spritePhotoMobile.getPositionX() + m*30;
	...
	initPlayerCardsPosition:function(){
		...
		for(var i=0;i<tmp_allplayers.length;i++){
			...
			if(player.isPower > 1){
				...
				this.setPlayerCardsPosition(player,player.myCards.length);
				for(var j = 0;j < player.myCards.length;j++){
					if(j < g_fapaiNum - 1){
						player.myCards[j].spriteBack.runAction(cc.sequence(cc.show()));
					}else{
						player.myCards[j].sprite.runAction(cc.sequence(cc.show()));
					}
				}
			}
		}
	...
	pomelo_on:function(){
		pomelo.on('onFapai',onFapai_function=function(data){
			...
			if(instruction_faPai=="fapaile!"){
				...
				//开始发牌的位置
				this.startFaPaiPosition = data["location"];;
				this.startFaPaiPosition_t = data["location"];;
				...
				for(var i=0;i<g_players.length;i++){
					...
					g_players[i].initPlayerCards(g_fapaiNum,"null");
				...
				for(var i=0;i<g_players.length;i++){
					...
					if(player.positionServer == this.startFaPaiPosition){
		...
		//继续收牌的动作
		pomelo.on('onFapaiNext',onFapaiNext_function=function(data){
			console.log("onFapaiNext" + JSON.stringify(data));
			this.jiaZhuMenuFlag = true;
			//当前玩家的标记
			...
			for(var i=0;i<g_players.length;i++){
				...
				if(player.positionServer == data["location"]){
					this.startFaPaiPosition = player.positionServer;
					this.startFaPaiPosition_t = player.positionServer;
					//玩家下注动作
					player.resetMoneyLabel(parseInt(data["my_gold"]));
					if(player.myCards.length >= 4){
						this.biPaiMenuItemFlag = true;
					}
					this.actionBottomBet(player.spritePhotoMobile.getPosition());
					console.log("add player card: startFaPaiPosition_t:" + this.startFaPaiPosition);
					break;
				}
			}
			...
			var callFuncActionFaPaiNext=cc.callFunc(this.actionFaPaiOne,this,card);
			g_dealCardBack.runAction(cc.sequence(new cc.EaseOut(acMoveDown,20),callFuncActionFaPaiNext));
		}.bind(this));

		pomelo.on('onFapaiContinue',onFapaiContinue_function=function(data){
			console.log("onFapaiContinue" + JSON.stringify(data));
			//跟牌玩家不可以加注
			this.jiaZhuMenuFlag = false;
			this.bet = data["chip"];
			this.sumBet = data["all_chip"];
			this.currentGetPowerPlayerPosition = data["first"];
			//当前玩家的标记
			/*新增玩家手中的牌（背面），权限isPower,开牌checkCard弃牌abandon,失败提示精灵loserSprite*/
			for(var i=0;i<g_players.length;i++){
				var player = g_players[i];
				if(player.positionServer == data["location"]){
					this.startFaPaiPosition = player.positionServer;
		...
		pomelo.on('onAdd',onAdd_function=function(data){
			...
			if(playerInfo["id"] != playerId){
				...
				t_player.push(playerInfo["paiXing"]);
				if(this.roomState==0){
					player = new TDKPlayers(t_player);
					g_players.push(player);
				}else{
					player = new TDKPlayers(t_player);
					g_players_noPower.push(player);
		...
		pomelo.on('onBipai',onBipai_function=function(data){
			...
			for(var i=0;i<tmp_allplayers.length;i++){
				...
				if(player.positionServer == playerPositionServer){
					player.setSpriteStatus("bipai");
					player.resetMoneyLabel(player.myGold - this.bet);
					this.actionAddBet(player.spritePhotoMobile.getPosition());
				}
				player.myScore = allScore[player.positionServer - 1][1];
				player.statusTag = "loser";
				if(player.positionServer == winnerPositionServer){
					player.statusTag = "win";
				}
			}
			//执行比牌动画
			//this.actionBiPai(winnerPositionServer);
		}.bind(this));

		pomelo.on('onEqual',onEqual_function=function(data){
			console.log("onEqual:" + JSON.stringify(data));
			var winnerPositionServer=data["winner"];
			var allPai = data["allpai"];
			this.currentGetPowerPlayerPosition = data["location"];
			this.shaiZiNum = data["shazi"];
			this.sumBet = data["all_chip"];
			console.log("winnerPositionServer:"+winnerPositionServer + " shaizi:" + this.shaiZiNum);
			//接收所有的牌 用于结束的时候打开所有的牌
			...
			for(var i=0;i<g_players.length;i++){
				...
				console.log("start shoupai card ......." + player.positionServer);
				var cardType = allPai[player.positionServer - 1];
				console.log("start shoupai card ......." + JSON.stringify(cardType));
				if(cardType == null || cardType == "null"){
					continue;
				}
				for(var j = 1;j < g_fapaiNum;j++){
					var suit=cardType["s" + j];
					var rank=cardType["p" + j];
					var position = this.getPlayerCardPosition(player,j - 1);
					player.myCards[j - 1].initCardSprite(parseInt(suit),parseInt(rank),position);
				}
			}
			//把赢家的牌放在桌面一次以便于瞻仰-_-
			//置房间状态为比牌状态并定时重置回来
			...
			for(var i=0;i<tmp_allplayers.length;i++){
				if(tmp_allplayers[i].positionServer == this.currentGetPowerPlayerPosition){
					this.actionAddBet(tmp_allplayers[i].spritePhotoMobile.getPosition());
					break;
				}
			}

			//执行比牌动画
			this.actionEqual(winnerPositionServer);
		}.bind(this));
					...
					if(this.comparableState==true){
						...
						var callbackActionWinnerGetBet=
							cc.callFunc(this.actionWinnerGetBet,this,
									player.spritePhotoMobile.getPosition());
		...
		pomelo.on('onFollow',onFollow_function=function(data){
			...
			for(var i=0;i<all_players.length;i++){
				...
				if(player.id == m_playerId){
					player.resetMoneyLabel(parseInt(data["my_gold"]));
					this.actionFollowBet(player.spritePhotoMobile.getPosition());
					break;
				}
			}
		}.bind(this));
		...
		pomelo.on('onShoupai',onShoupai_function=function(data){
			...
			this.currentGetPowerPlayerPosition = data["first"];
			this.myselfCards=new Array();
			for(i = 0;i < g_fapaiNum;i++){
				var suit=cardType["s" + (i + 1)];
				var rank=cardType["p" + (i + 1)];
				var card = new Array();
				card.push(suit);
				card.push(rank);
				this.myselfCards.push(card);
			...
			var size=cc.director.getWinSize();
			var tmp_allplayers = g_players_noPower.concat(g_players);
			console.log("start shoupai players length ......." + tmp_allplayers.length);
			for(var i=0;i<tmp_allplayers.length;i++){
				var player = tmp_allplayers[i];
				console.log("start shoupai card ......." + player.id);
				if(player.id == playerId){
					for(j = 0; j < g_fapaiNum;j++){
						var card = this.myselfCards[j];
						var position = this.getPlayerCardPosition(player,j);
						player.myCards[j].initCardSprite(parseInt(card[0]),parseInt(card[1]),position);
					}
					break;
				}
			}
			...
			for(var i=0;i<tmp_allplayers.length;i++){
				...
				for(var j = 0;j < tailPai.length;j++){
					var card = tailPai[j];
					if(card["location"] == player.positionServer && player.positionServer != g_myselfPlayerPos){
						console.log("start open the other tail card ......." + JSON.stringify(card));
						var position = this.getPlayerCardPosition(player,g_fapaiNum - 1);
						player.myCards[g_fapaiNum - 1].initCardSprite(parseInt(card["s"]),parseInt(card["p"]),position);
						break;
					}
				}
			}
			...
			var size=cc.director.getWinSize();
			var acMoveDown=cc.moveTo(0.5,cc.p(size.width/2,size.height));
			console.log("startFaPaiPosition:" + this.startFaPaiPosition);
			...
			if(g_players.length>=2) {
				...
				for (var i = 0; i < g_players.length; i++) {
					if (g_players[i].positionServer == rotationPlayerPositionServer) {
						console.log("rotationPlayerPositionServer:" + rotationPlayerPositionServer + " " + g_players[i].checkCard);
						g_players[i].counterTimer.startCounterTimer();
						break;
					}
				}
					...
					for(var j=0;j<tmp_allplayers.length;j++){
						...
						if(player.positionServer == i){
							for(var m = 0;m < g_fapaiNum - 1;m++){
								...
								player.myCards[m].setCardSpritePosition(player.playerPosition,parseInt(suit),parseInt(rank));
	...
	actionFaPai:function (){
    	...
    	console.log("start into actionFaPai........startFaPaiPosition:" + this.startFaPaiPosition);
		console.log("start into actionFaPai........startFaPaiPosition_t:" + this.startFaPaiPosition_t);
		console.log("start into actionFaPai........round_num:" + this.round_num);
    	if(this.startFaPaiPosition==this.startFaPaiPosition_t){this.round_num++}
    	if(this.round_num > g_fapaiNum){
    		this.round_num = 0;
    		this.startFaPaiPosition_t = this.startFaPaiPosition;
    		this.dealCardState=false;
    		var acMoveUp=cc.moveBy(0.5,cc.p(0,g_dealCardBack.getContentSize().height/2));
    		var acMoveUp1=new cc.EaseIn(acMoveUp,10);
    		g_dealCardBack.runAction(cc.sequence(acMoveUp1));
			//玩家自己的牌已经到达 掀开 最后一张牌
			var turn_num = g_players.length;
			var startFaPaiPosition_tmp = this.startFaPaiPosition;
			while(true){
				if(turn_num <= 0){
					break;
				}
				for(var i = 0;i < g_players.length;i++){
					var player = g_players[i];
					if(player.positionServer == startFaPaiPosition_tmp){
						//翻开最后一张牌
						var backCardSeq=cc.sequence(cc.delayTime(0.45),cc.hide());
						var backCamera=cc.orbitCamera(0.45,1,0,0,-90,0,0);
						var backSpawn=cc.spawn(backCardSeq,backCamera);
						...
						var frontCamera=cc.orbitCamera(0.6,1,0,0,-360,0,0);
						var frontSpawn=cc.spawn(frontSeq,frontCamera);
						player.myCards[g_fapaiNum - 1].spriteBack.runAction(backSpawn);
						player.myCards[g_fapaiNum - 1].sprite.runAction(frontSpawn);
						turn_num = turn_num - 1;
				...
				startFaPaiPosition_tmp = startFaPaiPosition_tmp + 1;
				if(startFaPaiPosition_tmp > 5){
					startFaPaiPosition_tmp = startFaPaiPosition_tmp%5;
				}
			}
    		//开启第一个轮换位置
		...
		for(var i = 0;i < g_players.length;i++){
			var player = g_players[i];
			if(player.positionServer == this.startFaPaiPosition_t){
    	...
    	if(isFind==true){
    		this.startFaPaiPosition_t++;
			if(this.startFaPaiPosition_t > 5){
				this.startFaPaiPosition_t = this.startFaPaiPosition_t % 5;
			}
    		...
    		if(player.playerPosition == 1){
				x = player.spritePhotoMobile.getPositionX() +
						player.spritePhotoMobile.getContentSize().width +
						g_dealCardBack.getContentSize().width/2*(this.round_num-1);
				y = player.spritePhotoMobile.getPositionY();
			}else if(player.playerPosition == 2){
				x = player.spritePhotoMobile.getPositionX() - (6 - this.round_num)*30;
				...
				x = player.spritePhotoMobile.getPositionX() + (this.round_num-1)*30;
				y =	player.spritePhotoMobile.getPositionY() - player.spritePhotoMobile.getContentSize().height - 30;
    		...
    		player.myCards[this.round_num-1].spriteBack.runAction(cc.sequence(cc.show(),acMoveDown1,acToCardPlayer,callFunc));
    	}
    	else{
    		this.startFaPaiPosition_t++;
			if(this.startFaPaiPosition_t > 5){
	...
	actionFaPaiOne:function (mythis,card){
		var size=cc.director.getWinSize();
    	console.log("start into actionFaPaiOne........startFaPaiPosition:" + this.startFaPaiPosition);
		console.log("start into actionFaPaiOne........card:" + JSON.stringify(card));
		
		for(var i = 0;i < g_players.length;i++){
			var player = g_players[i];
			if(player.positionServer == this.startFaPaiPosition_t){
				player.addPlayerCard();
				this.round_num = player.myCards.length;
				this.setPlayerCardBackPosition(player,this.round_num - 1);
				var position = this.getPlayerCardPosition(player,this.round_num - 1);
				var acMoveDown1=cc.moveTo(0.1,cc.p(size.width/2,size.height/2));
				var acToCardPlayer=cc.moveTo(0.1,position);
				player.myCards[this.round_num - 1].spriteBack.runAction(cc.sequence(acMoveDown1,acToCardPlayer));
		...
		for(var i = 0;i < g_players.length;i++){
			...
			if(player.positionServer == this.startFaPaiPosition){
				...
				player.myCards[player.myCards.length - 1].initCardSprite(parseInt(card[0]),parseInt(card[1]),position);
				...
				var frontCamera=cc.orbitCamera(0.6,1,0,0,-360,0,0);
				var frontSpawn=cc.spawn(frontSeq,frontCamera);
				player.myCards[player.myCards.length-1].spriteBack.runAction(backSpawn);
				player.myCards[player.myCards.length-1].sprite.runAction(frontSpawn);
				break;
			}
		}
		this.startFirstRotationPosition();
	...
	actionBiPai:function (winnerPositionServer){
		console.log("go into actionBiPai ......." + winnerPositionServer);
		var size=cc.director.getWinSize();
		var playerPostionId = -1;
		//把获胜的牌放在桌面中间
		for(var i=0;i<g_players.length;i++){
			var player = g_players[i];
			if(player.positionServer == winnerPositionServer){
				playerPostionId = i;
				console.log("found playerPostionId" + playerPostionId);
				break;
			}
		}
		if(playerPostionId == -1){
			console.log("no found playerPostionId");
		}
		var player = g_players[playerPostionId];
		
		//如果是自己获胜 则直接把牌放到桌面上
		if(player.positionServer == g_myselfPlayerPos){
			for(var j = 0;j < 5;j++){
				var cardPosition = player.myCards[j].spriteBack.getPosition();
				console.log("j:" + j + " x:" + cardPosition.x + " y:" + cardPosition.y);
				if(player.checkCard == false && j < g_fapaiNum - 1){
					var backCardSeq=cc.sequence(cc.delayTime(0.45),cc.hide());
					var backCamera=cc.orbitCamera(0.45,1,0,0,-90,0,0);
					var backSpawn=cc.spawn(backCardSeq,backCamera);
					var frontSeq=cc.sequence(cc.delayTime(0.45),cc.show());
					...
					var frontSpawn=cc.spawn(frontSeq,frontCamera);
					//移动到桌面位置
					var moveToBiPaiPosition=cc.moveTo(1.3,cc.p(size.width/2-(80*(3-j)),size.height/2));
					var moveToBiPaiPosition1=cc.moveTo(1.3,cc.p(size.width/2-(80*(3-j)),size.height/2));
					//前半段整合动作
					...
					//移动到原来的位置
					var moveToOriginPosition=cc.moveTo(1.5,cardPosition);
					var moveToOriginPosition1=cc.moveTo(1.5,cardPosition);
					
					var _seqBack=cc.sequence(backCombinationAction_,cc.delayTime(2),moveToOriginPosition);
					var _seqFront=cc.sequence(frontCombinationAction_,cc.delayTime(2),moveToOriginPosition1);
					player.myCards[j].sprite.runAction(_seqFront);
					player.myCards[j].spriteBack.runAction(_seqBack);
				}else{
					console.log("move idx:" + j + " pos:" + cardPosition);
					//移动的动作
					var moveToBiPaiPosition=cc.moveTo(1.3,cc.p(size.width/2-(80*(3-j)),size.height/2));
					var moveToBiPaiPosition1=cc.moveTo(1.3,cc.p(size.width/2-(80*(3-j)),size.height/2));
					//移动到原来的位置
		...
		var setDisplayWinnerCallback=cc.callFunc(this.displayWinner,this,winnerPositionServer);
		this.runAction(cc.sequence(displayWaitTime,setDisplayWinnerCallback));
		console.log("end go into displayLoser .......");
	},

	actionEqual:function (winnerPositionServer){
		console.log("go into actionBiPai ......." + winnerPositionServer);
		var size=cc.director.getWinSize();
		var winerArray = winnerPositionServer.split("*");
		//打开和牌的玩家的牌
		for(var i=0;i<g_players.length;i++){
			var player = g_players[i];
			if(player.positionServer == g_myselfPlayerPos){
				if(player.checkCard == false){
					for(var j = 0;j < g_fapaiNum - 1;j++){
						...
						var _seqBack=cc.sequence(backSpawn);
						var _seqFront=cc.sequence(frontSpawn);
						player.myCards[j].sprite.runAction(_seqFront);
						player.myCards[j].spriteBack.runAction(_seqBack);
					}
				}
		...
		console.log("start go into displayLoser .......");
		var displayWaitTime=new cc.DelayTime(4.8);
		var displayEqualCallBack=cc.callFunc(this.displayEqual,this,winerArray);
		this.runAction(cc.sequence(displayWaitTime,displayEqualCallBack));
		console.log("end go into displayLoser .......");
	},
	...
	actionFollowBet:function(playerPosition){
		console.log("go into actionFollowBet......" + playerPosition);
		var size=cc.director.getWinSize();
		var countNumber=-1;
		for(var j=0;j<g_betArray.length;j++){
			if(this.bet==g_betArray[j]){
	...
	actionAddBet:function(playerPosition){
		var size=cc.director.getWinSize();
		var countNumber=-1;
		for(var j=0;j<g_betArray.length;j++){
			if(this.bet==g_betArray[j]){
	...
	openAllCard:function(){
		...
		for(var i=0;i<tmp_allplayers.length;i++){
			var player = tmp_allplayers[i];
			if(player.positionServer == g_myselfPlayerPos){
				if(player.checkCard==false && player.isPower > 1){
					for(var j=0;j<g_fapaiNum - 1;j++){
						var backCardSeq=cc.sequence(cc.delayTime(0.45),cc.hide());
						var backCamera=cc.orbitCamera(0.45,1,0,0,-90,0,0);
						var backSpawn=cc.spawn(backCardSeq,backCamera);
		...
		for(var i=0;i<tmp_allplayers.length;i++){
			var player = tmp_allplayers[i];
			if(player.positionServer!= g_myselfPlayerPos && player.isPower > 1 && player.checkCard==false){
				for(var j = 0;j<g_fapaiNum - 1;j++){
					var backCardSeq=cc.sequence(cc.delayTime(0.45),cc.hide());
					var backCamera=cc.orbitCamera(0.45,1,0,0,-90,0,0);
		...
		for(var i=0;i<tmp_allplayers.length;i++){
			var player = tmp_allplayers[i];
			for(var j=0;j<player.myCards.length;j++){
				player.myCards[j].spriteBack.runAction(cc.hide());
				player.myCards[j].sprite.runAction(cc.hide());
			}
		}
		var bipaiFinish = new PopUpTDKBiPaiFinish();
		bipaiFinish.setPlayerCards(tmp_allplayers);
		this.addChild(bipaiFinish,10);
	},

	displayWinner:function(mythis,winnerPositionServer){
		for(var i=0;i< g_players.length;i++){
			if(g_players[i].positionServer==winnerPositionServer){
				console.log("displayWinner：" + winnerPositionServer);
				g_players[i].setSpriteStatus("win");
			}else{
	...
	displayEqual:function(mythis,winerArray){
		for(var i = 0;i < winerArray.length;i++){
			for(var j=0;j< g_players.length;j++){
				if(g_players[j].positionServer == winerArray[i]){
					console.log("displayWinner：" + winerArray[i]);
					g_players[j].setSpriteStatus("equal");
					break;
				}
			}
		}
		this.orbitCameraShaizi(this,winerArray);
	},
	
	orbitCameraShaizi:function(mythis,winerArray){
		console.log("orbitCameraShaizi:" + this.shaiZiRound + JSON.stringify(winerArray));
		if(this.shaiZiRound >= 20){
			//旋转完毕获取随机的筛子数并显示出来
			var size = cc.director.getWinSize();
			this.shaiZiSprite.setTexture("res/shaizi/shaizi_" + this.shaiZiNum + ".png");
			
			var callEqual = cc.callFunc(this.game_equal,this,winerArray);
			this.shaiZiSprite.runAction(cc.sequence(cc.show(),cc.FadeOut(1),callEqual));
			this.shaiZiRound = 1;
			return false;
		}
		var idx = this.shaiZiRound % 11;
		if(this.shaiZiRound > 10){
			idx = (this.shaiZiRound + 2)% 11;
		}
		if(this.shaiZiSprite == null){
			this.shaiZiSprite = new cc.Sprite("res/shaizi/xuanzhuan" + idx + ".png");
			var size = cc.director.getWinSize();
			this.shaiZiSprite.setPosition(size.width/2,size.height/2);
			this.addChild(this.shaiZiSprite);
		}else{
			this.shaiZiSprite.setTexture("res/shaizi/xuanzhuan" + idx + ".png");
		}
		
		this.shaiZiRound = this.shaiZiRound + 1;
		var callShaizi = cc.callFunc(this.orbitCameraShaizi,this,winerArray);
		this.shaiZiSprite.runAction(cc.sequence(cc.show(),cc.delayTime(0.1),cc.hide(),callShaizi));
	},

	game_equal:function(mythis,winerArray){
		this.shaiZiSprite.removeFromParent();
		this.shaiZiSprite = null;
		var pos = this.currentGetPowerPlayerPosition;
		var first_idx = 0;
		var shaziNumt = this.shaiZiNum;
		console.log("end for shaizi :" + this.shaiZiNum + " pos:"  + pos + " winners:" + JSON.stringify(winerArray));
		while(true){
			//开始依次循环找到玩家的位置 并确定 下次发牌的开始玩家
			if(shaziNumt <= 0){
				break;
			}
			console.log("find the first fa pai:" + first_idx);
			for(var i = 0;i < winerArray.length;i++){
				if(winerArray[i] == pos){
					first_idx = winerArray[i];
					shaziNumt = shaziNumt - 1;
					break;
				}
			}
			pos = pos + 1;
		...
		if(first_idx != 0 && this.currentGetPowerPlayerPosition == g_myselfPlayerPos){
			pomelo.request(util.getGameRoute(),{
				process:"equal",
					location:g_myselfPlayerPos,
					firstFaPai:first_idx
				},function(data){
					console.log(data.msg);
			});
		}else{
			console.log("it is not work well");
		}
	},

	startFirstRotationPosition:function (){
		console.log("startFirstRotationPosition......" + this.currentGetPowerPlayerPosition);
		this.initMenuItemVisibleCallbackAfterDealCard();

		for(var i=0;i < g_players.length;i++){
			var player = g_players[i];
			if(player.positionServer == this.currentGetPowerPlayerPosition){
				console.log("positionServer:" + player.positionServer + player.checkCard);
				player.counterTimer.startCounterTimer();
				break;
			}
		}
	...
	pomelo_removeListener:function(){
		...
		pomelo.removeListener('onEqual',onEqual_function);
		pomelo.removeListener('onFapaiNext',onFapaiNext_function);
		pomelo.removeListener('onFapaiContinue',onFapaiContinue_function);
...
var TDKGameScene = cc.Scene.extend({
	onEnter:function(){
		this._super();
		var layer = new tdkGameLayer();
		this.addChild(layer);
	}
});
